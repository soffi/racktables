...
function renderPortsForObject ($object_id = 0)
{
	...
	echo '<option value=nameaddr>Name[space]Address table</option>';
	echo "</select>";
...
function renderIPAddressPortForwarding ($object_id=0)
{
	...
	echo "<input type=hidden name=page value='${pageno}'>\n";
	echo "<input type=hidden name=tab value='${tabno}'>\n";
	echo "<tr align='center'><td colspan=2><select name='proto'>";
	foreach ($dict as $chapter)
		if ($chapter['name'] == 'Protocols') 
			foreach ($chapter['word'] as $key => $value)
				echo "<option value='${key}'>${value}</option>";
	echo "</select><select name='localip'>";
