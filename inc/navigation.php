...
{
	global $tab, $root, $page, $remote_username, $trigger, $tabextraclass, $numeric_revision, $head_revision, $tabrev;
	...
	foreach ($tab[$pageno] as $tabidx => $tabtitle)
	{
		...
		if (!permitted ($pageno, $tabidx))
			continue;
		// Hide tabs which are not to be shown when not in head revision
		if ($numeric_revision != $head_revision and !in_array($tabidx, $tabrev[$pageno]))
			continue;
		...
		echo " href='".makeHref(array('page'=>$pageno, 'tab'=>$tabidx));
