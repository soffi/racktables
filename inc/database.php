...
function mkQuery($query, $close=false)
{
	global $dbxlink;
	...
	if ($result == NULL)
	{
		...
		showError ("SQL query '${query}' failed: " . $errorInfo[2]);
...
function lockTablesGetHead($tables)
{
	$query = "lock tables ";
	$first = true;
	...
	mkQuery($query, true);
	$result = mkQuery("select max(id) from Revision");
...
function getObjectList ($type_id = 0)
{
	...
	$query =
		...
		"where " .
		"RackObject.rev <= $revision and RackObject.expirerev > $revision and ".
		"objtype_id = '${type_id}' and RackObject.deleted = 'no' " .
...
function getObjectInfo ($object_id = 0)
{
	...
	$query =
		...
		"where " .
		"RackObject.rev <= $revision and RackObject.expirerev > $revision and ".
		"id = '${object_id}' and deleted = 'no' and chapter_name = 'RackObjectType' limit 1";
	$result = $dbxlink->query ($query);
	if ($result == NULL)
	{
...
function getObjectPortsAndLinks ($object_id = 0)
{
	...
	$query =
		...
		"where " .
		"RackObject.rev <= $revision and RackObject.expirerev > $revision and ".
		"chapter_name = 'PortType' and Port.object_id=${object_id} ".
...
function commitAddObject ($new_name, $new_label, $new_barcode, $new_type_id, $new_asset_no)
{
	...
	// We need to lock tables and re-request the head number

	$newhead = lockTablesGetHead(array('Revision', 'RackObject'));
	$result = mkQuery("select IFNULL(max(id),0)+1 from RackObject");
	$row = $result->fetch(PDO::FETCH_NUM);
	$new_id = $row[0];
	$result->closeCursor();

	mkQuery("insert into RackObject(id, name, label, barcode, objtype_id, asset_no, rev, expirerev) " .
		"values(${new_id}, ${new_name}, '${new_label}', ${new_barcode}, '${new_type_id}', ${new_asset_no}, ${newhead}, ".MAXREVISION.")", true);

	mkQuery("insert into Revision(id, timestamp, comment, user_id) ".
		"values(${newhead}, now(), '', 1)", true);
	unlockTables();
...
function commitUpdateObject ($object_id = 0, $new_name = '', $new_label = '', $new_barcode = '', $new_type_id = 0, $new_has_problems = 'no', $new_asset_no = '', $new_comment = '')
{
	...
	mkQuery("update RackObject set expirerev=${newhead} where expirerev=".MAXREVISION." and id='${object_id}'", true);
	mkQuery("insert into RackObject(id, name, label, barcode, objtype_id, asset_no, has_problems, comment, rev, expirerev) " .
		"values(${object_id}, ${new_name}, '${new_label}', ${new_barcode}, '${new_type_id}', ${new_asset_no}, '${new_has_problems}', '${new_comment}', ${newhead}, ".MAXREVISION.")", true);
