...
CREATE VIEW RackSpace__vr as select id, max(rev) as rev from RackSpace__r group by id;

CREATE VIEW RackSpace as 
	SELECT 
	...
	FROM 
		RackSpace__r 
		JOIN RackSpace__vr ON RackSpace__r.id = RackSpace__vr.id and RackSpace__r.rev = RackSpace__vr.rev 
...
CREATE VIEW Rack__vr as select id, max(rev) as rev from Rack__r group by id;
...
CREATE VIEW Rack as 
	...
	FROM 
		Rack__r 
		JOIN Rack__vr ON Rack__r.id = Rack__vr.id and Rack__r.rev = Rack__vr.rev 
...
CREATE VIEW RackRow__vr as select id, max(rev) as rev from RackRow__r group by id;

CREATE VIEW RackRow as 
	SELECT 
		RackRow__s.id AS id ,
		RackRow__r.name AS name  
	FROM 
		RackRow__r 
		JOIN RackRow__vr ON RackRow__r.id = RackRow__vr.id and RackRow__r.rev = RackRow__vr.rev 
...
CREATE VIEW RackObject__vr as select id, max(rev) as rev from RackObject__r group by id;

CREATE VIEW RackObject as 
	SELECT 
	...
	FROM 
		RackObject__r 
		JOIN RackObject__vr ON RackObject__r.id = RackObject__vr.id and RackObject__r.rev = RackObject__vr.rev 
...
CREATE VIEW Port__vr as select id, max(rev) as rev from Port__r group by id;
...
CREATE VIEW Port as 
	...
	FROM 
		Port__r 
		JOIN Port__vr ON Port__r.id = Port__vr.id and Port__r.rev = Port__vr.rev 
...
CREATE VIEW Link__vr as select id, max(rev) as rev from Link__r group by id;
...
CREATE VIEW Link as 
	...
	FROM 
		Link__r 
		JOIN Link__vr ON Link__r.id = Link__vr.id and Link__r.rev = Link__vr.rev 
...
CREATE VIEW IPv4Address__vr as select id, max(rev) as rev from IPv4Address__r group by id;

CREATE VIEW IPv4Address as 
	SELECT 
	...
	FROM 
		IPv4Address__r 
		JOIN IPv4Address__vr ON IPv4Address__r.id = IPv4Address__vr.id and IPv4Address__r.rev = IPv4Address__vr.rev 
...
CREATE VIEW IPv4Allocation__vr as select id, max(rev) as rev from IPv4Allocation__r group by id;

CREATE VIEW IPv4Allocation as 
	SELECT 
	...
	FROM 
		IPv4Allocation__r 
		JOIN IPv4Allocation__vr ON IPv4Allocation__r.id = IPv4Allocation__vr.id and IPv4Allocation__r.rev = IPv4Allocation__vr.rev 
...
CREATE VIEW IPv4NAT__vr as select id, max(rev) as rev from IPv4NAT__r group by id;

CREATE VIEW IPv4NAT as 
	SELECT 
		IPv4NAT__s.id AS id ,
	...
	FROM 
		IPv4NAT__r 
		JOIN IPv4NAT__vr ON IPv4NAT__r.id = IPv4NAT__vr.id and IPv4NAT__r.rev = IPv4NAT__vr.rev 
...
CREATE VIEW IPv4Network__vr as select id, max(rev) as rev from IPv4Network__r group by id;

CREATE VIEW IPv4Network as 
	SELECT 
	...
	FROM 
		IPv4Network__r 
		JOIN IPv4Network__vr ON IPv4Network__r.id = IPv4Network__vr.id and IPv4Network__r.rev = IPv4Network__vr.rev 
...
CREATE VIEW Attribute__vr as select id, max(rev) as rev from Attribute__r group by id;
...
CREATE VIEW Attribute as 
	...
	FROM 
		Attribute__r 
		JOIN Attribute__vr ON Attribute__r.id = Attribute__vr.id and Attribute__r.rev = Attribute__vr.rev 
...
CREATE VIEW AttributeMap__vr as select id, max(rev) as rev from AttributeMap__r group by id;

CREATE VIEW AttributeMap as 
	SELECT 
	...
	FROM 
		AttributeMap__r 
		JOIN AttributeMap__vr ON AttributeMap__r.id = AttributeMap__vr.id and AttributeMap__r.rev = AttributeMap__vr.rev 
...
CREATE VIEW AttributeValue__vr as select id, max(rev) as rev from AttributeValue__r group by id;

CREATE VIEW AttributeValue as 
	SELECT 
	...
	FROM 
		AttributeValue__r 
		JOIN AttributeValue__vr ON AttributeValue__r.id = AttributeValue__vr.id and AttributeValue__r.rev = AttributeValue__vr.rev 
...
CREATE VIEW Dictionary__vr as select id, max(rev) as rev from Dictionary__r group by id;
...
CREATE VIEW Dictionary as 
	...
	FROM 
		Dictionary__r 
		JOIN Dictionary__vr ON Dictionary__r.id = Dictionary__vr.id and Dictionary__r.rev = Dictionary__vr.rev 
...
CREATE VIEW Chapter__vr as select id, max(rev) as rev from Chapter__r group by id;
...
CREATE VIEW Chapter as 
	...
	FROM 
		Chapter__r 
		JOIN Chapter__vr ON Chapter__r.id = Chapter__vr.id and Chapter__r.rev = Chapter__vr.rev 
...
CREATE VIEW IPv4LB__vr as select id, max(rev) as rev from IPv4LB__r group by id;

CREATE VIEW IPv4LB as 
	SELECT 
		IPv4LB__s.id AS id ,
	...
	FROM 
		IPv4LB__r 
		JOIN IPv4LB__vr ON IPv4LB__r.id = IPv4LB__vr.id and IPv4LB__r.rev = IPv4LB__vr.rev 
...
CREATE VIEW IPv4RS__vr as select id, max(rev) as rev from IPv4RS__r group by id;

CREATE VIEW IPv4RS as 
	SELECT 
		IPv4RS__s.id AS id ,
	...
	FROM 
		IPv4RS__r 
		JOIN IPv4RS__vr ON IPv4RS__r.id = IPv4RS__vr.id and IPv4RS__r.rev = IPv4RS__vr.rev 
...
CREATE VIEW IPv4RSPool__vr as select id, max(rev) as rev from IPv4RSPool__r group by id;

CREATE VIEW IPv4RSPool as 
	SELECT 
	...
	FROM 
		IPv4RSPool__r 
		JOIN IPv4RSPool__vr ON IPv4RSPool__r.id = IPv4RSPool__vr.id and IPv4RSPool__r.rev = IPv4RSPool__vr.rev 
...
CREATE VIEW IPv4VS__vr as select id, max(rev) as rev from IPv4VS__r group by id;

CREATE VIEW IPv4VS as 
	SELECT 
		IPv4VS__s.id AS id ,
	...
	FROM 
		IPv4VS__r 
		JOIN IPv4VS__vr ON IPv4VS__r.id = IPv4VS__vr.id and IPv4VS__r.rev = IPv4VS__vr.rev 
...
CREATE VIEW TagStorage__vr as select id, max(rev) as rev from TagStorage__r group by id;

CREATE VIEW TagStorage as 
	SELECT 
	...
	FROM 
		TagStorage__r 
		JOIN TagStorage__vr ON TagStorage__r.id = TagStorage__vr.id and TagStorage__r.rev = TagStorage__vr.rev 
...
CREATE VIEW TagTree__vr as select id, max(rev) as rev from TagTree__r group by id;

CREATE VIEW TagTree as 
	SELECT 
		TagTree__s.id AS id ,
	...
	FROM 
		TagTree__r 
		JOIN TagTree__vr ON TagTree__r.id = TagTree__vr.id and TagTree__r.rev = TagTree__vr.rev 
...
CREATE VIEW FileLink__vr as select id, max(rev) as rev from FileLink__r group by id;

CREATE VIEW FileLink as 
	SELECT 
	...
	FROM 
		FileLink__r 
		JOIN FileLink__vr ON FileLink__r.id = FileLink__vr.id and FileLink__r.rev = FileLink__vr.rev 
...
CREATE VIEW File__vr as select id, max(rev) as rev from File__r group by id;
...
CREATE VIEW File as 
	...
	FROM 
		File__r 
		JOIN File__vr ON File__r.id = File__vr.id and File__r.rev = File__vr.rev 
